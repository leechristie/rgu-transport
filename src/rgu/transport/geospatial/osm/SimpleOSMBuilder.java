 ...
 */
public final class SimpleOSMBuilder implements OSMListener {

    final Metric<GeoLocation, Distance> metric = GeoLocation.HAVERSINE;
    final Map<Long, GeoLocation> locations = new HashMap<>(1_000_000);
    final Map<GeoLocation, Long> reverse = new HashMap<>(1_000_000);
    final HashGraph.Builder<GeoLocation, Duration> builder = HashGraph.builder();
    ...
    public static final BiPredicate<Speed, Map<String, String>> IS_HIGHWAY
        = (_, tags) -> tags.containsKey("highway");
    ...
    public SimpleOSMBuilder(Speed maxSpeed,
                            BiPredicate<Speed, Map<String, String>> edgeFilter,
                            Predicate<GeoLocation> vertexFilter) {
        this(maxSpeed, edgeFilter, vertexFilter, null);
    }
    ...
    public SimpleOSMBuilder(Speed maxSpeed,
        ...
        Objects.requireNonNull(maxSpeed, "maxSpeed");
    ...
    public Graph<GeoLocation, Duration> build() {
        this.progress.done();
    ...
    public void way(long id, String version, LocalDateTime timestamp,
        ...
        for (long currentID : points) {
            ...
            if (previous != null) {
                ...
                if (speed.compareTo(this.maxSpeed) > 0) {
                    speed = this.maxSpeed;
                }
                Duration time = Duration.ofSeconds(
                        (long) (distance.asMeters()
                              / speed.asMetersPerSecond()));
                ...
                if (bothTest) {
                    try {
                        this.builder.addEdge(previous, current, time);
                        added = true;
                    } catch (IllegalStateException ex) {
                        // ignored
                    }
                    try {
                        this.builder.addEdge(current, previous, time);
                        added = true;
                    } catch (IllegalStateException ex) {
                        // ignored
                    }
